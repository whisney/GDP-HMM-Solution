...
def NiiDataRead_PA(path, shape, as_type=np.float32):
    try:
        volumn, spacing_, origin, direction = NiiDataRead(path, as_type=as_type)
    except:
        volumn = np.zeros(shape)
        spacing_ = None
        origin = None
        direction = None
    return volumn, spacing_, origin, direction